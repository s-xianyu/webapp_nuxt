<template>
  <div class="city" :class="{cur:cityStatus}">
...
<script>
  ...
  export default {
    ...
    computed:{
      ...mapState(['historyCity','cityStatus'])
    },
    components:{
      TitleHead
    },
    ...
    methods:{
      ...
      ...mapMutations(['CITY_SAVE','ALL_CITY','WIN_HEIGHT','ALL_STATUS']),
      async getLocation(){
        let params = { keytype:0 };
        let { data } = await historyKeyAndHotKey(params);
        this.locationCity = data.hotKeys[1];
      },
      Scrolls(key){
